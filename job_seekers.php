...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (isset($_POST['id']) && $_POST['id'] !== '') {
		...
		$types .= 'i';
		$values[] = $id;
		$sql = 'UPDATE job_seekers SET ' . implode(', ', $set) . ' WHERE id=?';
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
	...
	header('Content-Disposition: attachment; filename="job_seekers_' . date('Ymd_His') . '.csv"');
...
// Count total
$sqlCount = 'SELECT COUNT(*) FROM job_seekers ' . $where;
		...
		<div class="card-body">
			<nav>
				<ul class="pagination mb-0">
					<?php $base = 'job_seekers.php?search=' . urlencode($search) . '&page='; ?>
