...
function jpa_schema_column_definition(mysqli $conn, string $table, string $column): string
{
...
function jpa_ensure_schema(mysqli $conn): void
{
    ...
    $conn->query("CREATE TABLE IF NOT EXISTS job_portal_award_red_flags (
        ...
        code ENUM('RF-1','RF-2','RF-3') NOT NULL,
    ...
    $redFlagCodeDefinition = jpa_schema_column_definition($conn, 'job_portal_award_red_flags', 'code');
    if (strpos($redFlagCodeDefinition, "'rf-4'") !== false || strpos($redFlagCodeDefinition, "'rf-3'") === false) {
        $conn->query("ALTER TABLE job_portal_award_red_flags MODIFY code ENUM('RF-1','RF-2','RF-3','RF-4') NOT NULL");
        $conn->query("UPDATE job_portal_award_red_flags SET code='RF-3' WHERE code='RF-4'");
        $conn->query("ALTER TABLE job_portal_award_red_flags MODIFY code ENUM('RF-1','RF-2','RF-3') NOT NULL");
    }
