...
if (!(current_user_can('settings_virtual_karir_service_manage') || current_user_can('manage_settings'))) { http_response_code(403); echo 'Forbidden'; exit; }
...
if (isset($_POST['add'])) {
    ...
    $link = $_POST['link'];
    $stmt = $conn->prepare("INSERT INTO virtual_karir_services (icon, title, description, link, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())");
...
if (isset($_POST['update'])) {
    ...
    $description = $_POST['description'];
    $link = $_POST['link'];
    $stmt = $conn->prepare("UPDATE virtual_karir_services SET icon=?, title=?, description=?, link=?, updated_at=NOW() WHERE id=?");
...
if (isset($_GET['delete'])) {
    $id = $_GET['delete'];
    $stmt = $conn->prepare("DELETE FROM virtual_karir_services WHERE id=?");
...
if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $result = $conn->query("SELECT * FROM virtual_karir_services WHERE id=$id");
    $edit_service = $result->fetch_assoc();
}

// Fetch all services
$services = $conn->query("SELECT * FROM virtual_karir_services ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Virtual Karir Service Settings</title>
...
<body class="bg-light">
     ...
     <!-- End Navigation Bar -->
    <div class="container">
        <h2>Virtual Karir Service Settings</h2>
        ...
        <form method="post">
            ...
            <?php endif; ?>
            <label>Icon:
                <input type="text" name="icon" value="<?php echo $edit_service['icon'] ?? ''; ?>">
            </label>
            <label>Title:
            ...
            <label>Link:
                <input type="text" name="link" value="<?php echo $edit_service['link'] ?? ''; ?>">
            </label>
