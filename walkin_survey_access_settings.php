...
$conn->query("CREATE TABLE IF NOT EXISTS walkin_survey_access_settings (
...
$resInit = $conn->query("SELECT id FROM walkin_survey_access_settings WHERE id = 1 LIMIT 1");
if (!$resInit || $resInit->num_rows === 0) {
    $defaultHash = password_hash('123456', PASSWORD_BCRYPT);
    $stmtInit = $conn->prepare("INSERT INTO walkin_survey_access_settings (id, is_enabled, passcode_hash) VALUES (1, 1, ?)");
    if ($stmtInit) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $current = $conn->query("SELECT passcode_hash FROM walkin_survey_access_settings WHERE id = 1 LIMIT 1");
    ...
    $stmt = $conn->prepare("UPDATE walkin_survey_access_settings SET is_enabled = ?, passcode_hash = ? WHERE id = 1");
    if ($stmt) {
        $stmt->bind_param('is', $isEnabled, $hash);
        $stmt->execute();
        $stmt->close();
        $_SESSION['success'] = 'Pengaturan akses Survei Evaluasi berhasil disimpan.';
    } else {
...
$res = $conn->query("SELECT is_enabled, updated_at FROM walkin_survey_access_settings WHERE id = 1 LIMIT 1");
...
<head>
    ...
    <title>Walk-in Survey Access Settings</title>
                ...
                <div class="col-12">
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="is_enabled">
                            Aktifkan passcode untuk menu <strong>Survei Evaluasi</strong> di halaman Kemitraan
                        </label>
                    </div>
                    <div class="form-text">Jika aktif, user wajib memasukkan passcode yang benar saat menekan tab Survei Evaluasi.</div>
                </div>
