...
$columns = [];
$rawCols = [];
if (!$result = $conn->query('SHOW COLUMNS FROM jobs')) {
	die('Failed to read jobs schema');
}
...
foreach ($rawCols as $col) {
	...
	$columns[$field] = ['label' => to_label($field), 'type' => $inputType];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (isset($_POST['id']) && $_POST['id'] !== '') {
		...
		$types = '';
		$values = [];
		foreach ($fields as $f) { $set[] = "$f=?"; $types .= 's'; $values[] = $_POST[$f] ?? null; }
		$types .= 'i';
		$values[] = $id;
...
// Build searchable fields (prefer common fields if present)
$preferredSearch = ['job_id','nama_jabatan','nama_perusahaan','provinsi','kab_kota','platform_lowongan'];
$searchable = array_values(array_intersect($preferredSearch, array_keys($columns)));
if (count($searchable) === 0) {
	$searchable = array_slice(array_keys($columns), 0, min(5, count($columns)));
}
...
if (isset($_GET['export']) && $_GET['export'] === '1') {
	...
	$sql = 'SELECT id, ' . implode(', ', array_keys($columns)) . ' FROM jobs ' . $where . ' ORDER BY id DESC';
	$stmt = $conn->prepare($sql);
	if ($where !== '') { $stmt->bind_param($types, ...$params); }
	$stmt->execute();
	$res = $stmt->get_result();
	header('Content-Type: text/csv; charset=utf-8');
	header('Content-Disposition: attachment; filename="jobs_' . date('Ymd_His') . '.csv"');
	$out = fopen('php://output', 'w');
	$header = array_merge(['ID'], array_map(fn($k) => $columns[$k]['label'], array_keys($columns)));
	fputcsv($out, $header);
	while ($row = $res->fetch_assoc()) {
		$line = [$row['id']];
		foreach ($columns as $name => $_meta) { $line[] = $row[$name] ?? ''; }
		fputcsv($out, $line);
	}
	fclose($out);
	exit;
...
// Count total
$sqlCount = 'SELECT COUNT(*) FROM jobs ' . $where;
...
// Fetch page
$sql = 'SELECT id, ' . implode(', ', array_keys($columns)) . ' FROM jobs ' . $where . ' ORDER BY id DESC LIMIT ? OFFSET ?';
$stmt = $conn->prepare($sql);
if ($where !== '') {
...
<div class="container py-4">
	<div class="d-flex flex-wrap justify-content-between align-items-center mb-3">
		<h2 class="mb-2 mb-md-0">Jobs</h2>
		<div class="d-flex gap-2">
			<form class="d-flex" method="get" action="jobs.php">
			...
			<a class="btn btn-success" href="jobs.php?export=1<?php echo $search !== '' ? '&search=' . urlencode($search) : '';?>"><i class="bi bi-file-earmark-excel"></i> Export</a>
		</div>
	</div>
		...
		<div class="table-responsive">
			<table class="table table-striped mb-0">
				...
				<?php foreach ($rows as $row): ?>
					<tr>
						<td>
							<a class="btn btn-sm btn-outline-primary" href="jobs.php?action=edit&id=<?php echo $row['id']; ?>#edit">Edit</a>
							<a class="btn btn-sm btn-outline-danger" href="jobs.php?action=delete&id=<?php echo $row['id']; ?>" onclick="return confirm('Delete this record?');">Delete</a>
						</td>
		...
		<div class="card-body">
			<nav>
				<ul class="pagination mb-0">
					<?php $base = 'jobs.php?search=' . urlencode($search) . '&page='; ?>
	...
	<?php if ($action === 'edit' && isset($_GET['id'])):
		$id = intval($_GET['id']);
		$stmt = $conn->prepare('SELECT id, ' . implode(', ', array_keys($columns)) . ' FROM jobs WHERE id=?');
		$stmt->bind_param('i', $id);
		$stmt->execute();
	...
	<div id="edit" class="card mt-4">
		<div class="card-body">
			<h5 class="mb-3">Edit Job (ID: <?php echo $id; ?>)</h5>
			<form method="post">
